...
const typeMeta: Record<PostContentType, { label: string; icon: typeof PenLine; className: string }> = {
  BLOG: { label: 'Bài viết', icon: PenLine, className: 'post' },
  QUESTION: { label: 'Câu hỏi', icon: FileQuestion, className: 'question' },
  DISCUSSION: { label: 'Thảo luận', icon: MessagesSquare, className: 'discussion' },
...
function PostsPage() {
  ...
  const [posts, setPosts] = useState<Awaited<ReturnType<typeof getPosts>>['content']>([])
    ...
    async function loadPosts() {
      ...
      try {
        ...
        if (!controller.signal.aborted) {
          setPosts([])
          setPage(initialPage)
          setErrorMessage(getApiErrorMessage(error, 'Không thể tải danh sách bài viết.'))
        }
      } finally {
        if (!controller.signal.aborted) {
          setIsLoading(false)
        ...
        <div className="posts-layout">
          <section className="knowledge-feed" aria-labelledby="knowledge-title">
            <div className="knowledge-page-heading">
              ...
              <Link className="heading-create-button" to="/posts/new"><Plus size={16} /> Tạo bài viết</Link>
            </div>
            ...
            <div className="knowledge-topics" aria-label="Lọc theo tag">
              <button className={!activeTag ? 'is-active' : ''} type="button" onClick={() => changeTag('')}>Tất cả tags</button>
              {tags.map((tag) => (
                <button className={activeTag === tag.name ? 'is-active' : ''} type="button" onClick={() => changeTag(tag.name)} key={tag.id}>
                  {tag.name}
                </button>
              ))}
            </div>
            ...
            <div className="knowledge-list" aria-live="polite">
              {isLoading && Array.from({ length: 4 }, (_, index) => <div className="knowledge-card-skeleton" key={index} />)}
              ...
              {!isLoading && !errorMessage && posts.map((post) => {
                ...
                return (
                  <article className={`knowledge-card knowledge-card--${meta.className}`} key={post.id}>
                    ...
                    <div className="knowledge-card-content">
                      <div className="knowledge-labels">
                        ...
                        {post.tags.map((tag) => <span className="content-tag" key={tag.id}>{tag.name}</span>)}
                      </div>
                      <h2><Link to={`/posts/${post.id}`}>{post.title}</Link></h2>
                      <p>{authorName} · {formatPostDate(post.createdAt)}</p>
                    </div>
            ...
            {!isLoading && !errorMessage && page.totalPages > 1 && (
              <nav className="posts-pagination" aria-label="Phân trang bài viết">
                <button type="button" onClick={() => setPageNumber(page.number - 1)} disabled={page.number === 0} aria-label="Trang trước">
                  <ChevronLeft size={17} />
                </button>
                {visiblePages.map((pageIndex) => (
                  <button
                    ...
                    aria-current={pageIndex === page.number ? 'page' : undefined}
                    key={pageIndex}
                  >
                    {pageIndex + 1}
                  </button>
                ))}
                <button type="button" onClick={() => setPageNumber(page.number + 1)} disabled={page.number >= page.totalPages - 1} aria-label="Trang tiếp theo">
